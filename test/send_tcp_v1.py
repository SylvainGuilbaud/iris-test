...
def envoyer_message_tcp(message, host=TCP_HOST, port=TCP_PORT):
    """Envoie un message HL7 via TCP avec synchronisation IRIS"""
    sock = None
    try:
        ...
        # Pause pour établir la connexion proprement
        time.sleep(0.1)
        
        # Envoi du message avec wrapping HL7
        wrapped_message = f"\x0b{message}\x1c\x0d"
        sock.send(wrapped_message.encode('utf-8'))
        print(f"Message envoyé sur {host}:{port}")
        
        # Attendre la réponse d'IRIS (ACK) avant de continuer
        try:
            sock.settimeout(3)  # Timeout plus long pour IRIS
            ...
            print("⚠️ Timeout - pas de réponse d'IRIS")
        except Exception as e:
            print(f"⚠️ Erreur réception: {e}")
            
        # Pause avant fermeture pour s'assurer que tout est traité
        time.sleep(0.2)
        ...
        if sock:
            try:
                ...
                time.sleep(0.05)  # Pause avant close
                sock.close()
            except:
                pass
...
def main():
    ...
    try:
        ...
        for i in range(nombre_messages):
            ...
            # Délai suffisant pour éviter surcharge IRIS
            if i < nombre_messages - 1:
                print("Attente traitement IRIS...")
                time.sleep(0.5)
