...
def insert_patient_with_vector():
    ...
    try:
        ...
        # Create a sample 256-dimensional vector
        # Example 1: Random vector
        vector_data = [random.random() for _ in range(256)]
        ...
        embedding_vector = [0.1, 0.2, 0.3] + [0.0] * 253  # Fill to 256 dimensions
...
ClassMethod InsertPatientSQL() As %Status
{
    ...
    for i = 1:1:256 {
        ...
        if i < 256 set vector1 = vector1 _ ","
    }
...
INSERT INTO app.patient (name, comment)
VALUES ('Vector Patient', TO_VECTOR('""" + str([0.1 * i for i in range(256)]) + """', 'DOUBLE'));
    """)